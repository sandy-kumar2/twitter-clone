...
const Login = () => {
  ...
  return (
    <div className='w-screen h-screen flex items-center justify-content'>
      <div className='flex items-center justify-evenly w-[80%]'>
        <div>
          <img className='ml-5' width={"300px"} src="/twitterlogo.png" alt="twitter-logo" />
        </div>
        <div>
          <div className='my-5'>
            <h1 className='font-bold text-6xl'>What's Up Peeps</h1>
          </div>
          ...
          <form onSubmit={submitHandler} className='flex flex-col w-[55%]'>
            ...
            <button className='bg-[#1D9BF0] border-none py-2 my-4 rounded-full text-lg text-white'>{isLogin? "Login": "Create Your Account"}</button>
            <h1>{isLogin? "Do not have an Acoount?": "Already have an Account?"} <span onClick={loginSignupHandler} className='font-bold text-blue-600 cursor-pointer'>{isLogin? "Sign-Up": "Login"}</span></h1>
          </form>
        </div>
      </div>
    </div>
